...
export async function runVelogBatchAgent(params: {
  rows: SheetRow[];
  batchNum: number;
}): Promise<VelogBatchResult> {
  const result: VelogBatchResult = { posted: 0, failed: 0, results: [] };
  ...
  for (const row of params.rows) {
    ...
    const postResult = await postToVelogAccount(accountName, title, contentHtml);
...
async function postToVelogAccount(
  accountName: string,
  title: string,
  contentHtml: string,
): Promise<{ success: boolean; postUrl?: string; error?: string }> {
  try {
    const loginResult = await executeBrowserTool('login_velog', { nickname: accountName });
    if (!loginResult.success) {
      return { success: false, error: loginResult.error || 'Velog login failed' };
    }
    const postResult = await executeBrowserTool('post_velog', { title, htmlContent: contentHtml });
    return {
