...
export async function runCodaBatchAgent(params: {
  rows: SheetRow[];
  batchNum: number;
}): Promise<CodaBatchResult> {
  const result: CodaBatchResult = { posted: 0, failed: 0, results: [] };
  ...
  for (const row of params.rows) {
    ...
    const postResult = await postToCodaAccount(accountName, title, contentHtml);
...
async function postToCodaAccount(
  ...
  try {
    ...
    const postResult = await executeBrowserTool('post_coda', { title, htmlContent: contentHtml });
    return {
